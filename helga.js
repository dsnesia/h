...
// Start
module.exports = helga = async (helga, m, chatUpdate, store) => {
    try {
        ...
        const botNumber = helga.user.id ? helga.user.id.split(":")[0]+"@s.whatsapp.net" : helga.user.id
        ...
        const groupMetadata = m.isGroup ? await helga.groupMetadata(m.chat).catch(e => {}) : ''
...
if (budy.includes('https://chat.whatsapp.com/')) {
               ...
               let sianj = m.sender
               await helga.groupParticipantsUpdate(m.chat, [sianj], 'remove').then((res) => m.reply(jsonformat(res))).catch((err) => m.reply(jsonformat(err)))
               }
	  }
        // respon
     ...
     * @returns
     */
    helga.send5Vid = async (jid , text = '' , footer = '', vid, but = [], options = {}) =>{
        let message = await prepareWAMessageMedia({ video: vid }, { upload: helga.waUploadToServer })
        ...
        const textTemplateButtons = (from, text, footer, buttons) => {
            return helga.sendMessage(from, { text: text, footer: footer, templateButtons: buttons })
        }
       // Detect User Banned
        if (ban[m.sender] && isCmd) {
            await helga.sendText(m.chat, `Maaf @${m.sender.split("@")[0]} Anda Telah Dibanned, Chat Owner Untuk Un Banned`, m, { contextInfo: { mentionedJid: [m.sender] }})
            return
        }
...
user.afkReason = ''
        ...
        helga.sendReadReceipt(m.chat, m.sender, [m.key.id])
        helga.sendPresenceUpdate('composing', m.chat)
        if (isCmd && !isGroup) {
...
console.log(err)
        ...
        let messages = await generateWAMessage(m.chat, { text: text, mentions: mentionedJid }, {
            userJid: helga.user.id,
            quoted: m.quoted && m.quoted.fakeObj
        })
        messages.key.fromMe = areJidsSameUser(m.sender, helga.user.id)
        ...
        helga.ev.emit('messages.upsert', msg)
        }
...
${isSurender ? '' : ``}`.trim()
	    helga.sendText(m.chat, caption, m, { contextInfo: { mentionedJid: parseMention(caption) }}).then(mes => { return _family100['family100'+m.chat].pesan = mesg }).catch(_ => _)
...
helga.send5ButImg(m.chat, Anu, ` Helga Zexs妥 `, fatihgans, btn)
                                          break
case 'kick': {
...
break
            ...
            break
                 ...
                 for (let i of anu) {
                     let metadata = await helga.groupMetadata(i)
                     teks += `› *Nama :* ${metadata.subject}\n› *ID :* ${metadata.id}\n› *Dibuat :* ${moment(metadata.creation * 1000).tz('Asia/Jakarta').format('DD/MM/YYYY HH:mm:ss')}\n› *Member :* ${metadata.participants.length}\n\n────────────────────────\n\n`
                 }
                 helga.sendTextWithMentions(m.chat, teks, m)
             }
             break
                                          case 'termint':
let uname = args[0]
 ...
 if(from != "120363041199947223@g.us") return m.reply("maaf su fitur ini dapet di gunakan group tertentu")
        function subDomain1(host, ip) {
          return new Promise((resolve) => {
            ...
            let apiToken1 = "-DpZQl5D52wS7NxV7FzGNQkoWKvLZE4kg-PBf5OF";
            let tld1 = "dosg.my.id";
            axios
              .post(
...
m.reply(e)
		...
		if (isCmd && budy.toLowerCase() != undefined) {
		    ...
		    helga.copyNForward(m.chat, msgs[budy.toLowerCase()], true)
		}
        }
