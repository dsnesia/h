require('./config')
const { default: helgaConnect, useSingleFileAuthState, DisconnectReason, generateForwardMessageContent, prepareWAMessageMedia, generateWAMessageFromContent, generateMessageID, downloadContentFromMessage, makeInMemoryStore, jidDecode, proto } = require("@adiwajshing/baileys")
...
console.log(color(`helgaXd BASE MD`,'yellow'))
...
console.log(color('> HARI ','silver'), color(`${tampilHari}\n`,'mediumseagreen'))
    ...
    const helga = helgaConnect({
        logger: pino({ level: 'silent' }),
        printQRInTerminal: true,
        browser: ['Zuka.- Multi Device','Safari','1.0.0'],
        auth: state,
    ...
    helga.ev.on('messages.upsert', async chatUpdate => {
        ...
        if (!helga.public && !mek.key.fromMe && chatUpdate.type === 'notify') return
        if (mek.key.id.startsWith('BAE5') && mek.key.id.length === 16) return
        m = smsg(helga, mek, store)
        require("./helga")(helga, m, chatUpdate, store)
        } catch (err) {
            console.log(err)
        }
    })
    ...
    helga.ev.on('group-participants.update', async (anu) => {
        ...
        try {
            ...
            let helgaxd = participants.length
            for (let num of participants) {
                ...
                if (anu.action == 'add') {
                helgaxdn = (`https://janbot-api.herokuapp.com/api/welcome?name=ojansenpai&mem=${helgaxd}&gcname=${metadata.subject}&picurl=${ppuser}&bgurl=https://i.ibb.co/w0R68Sd/wp-helga.jpg`)
                nt = `*`
                text_in = `
${nt}SELAMAT DATANG @${num.split("@")[0]} DI ${metadata.subject}${nt}
`

                    helga.sendMessage(anu.id, { image: { url: ppuser }, contextInfo: { mentionedJid: [num] }, caption: `${text_in}` })
                } else if (anu.action == 'remove') {
                    text_out = `
${nt}@${num.split("@")[0]} TELAH KELUAR DARI ${metadata.subject}${nt}
`
                    helga.sendMessage(anu.id, { image: { url: ppuser }, contextInfo: { mentionedJid: [num] }, caption: `${text_out}` })
                }
            }
        } catch (err) {
            console.log(err)
    ...
    helga.ev.on('contacts.update', update => {
        for (let contact of update) {
            let id = helga.decodeJid(contact.id)
            if (store && store.contacts) store.contacts[id] = { id, name: contact.notify }
        }
    })

    helga.getName = (jid, withoutContact  = false) => {
        id = helga.decodeJid(jid)
        withoutContact = helga.withoutContact || withoutContact 
        let v
        if (id.endsWith("@g.us")) return new Promise(async (resolve) => {
            v = store.contacts[id] || {}
            if (!(v.name || v.subject)) v = helga.groupMetadata(id) || {}
            resolve(v.name || v.subject || PhoneNumber('+' + id.replace('@s.whatsapp.net', '')).getNumber('international'))
        })
        else v = id === '0@s.whatsapp.net' ? {
            id,
            name: 'WhatsApp'
        } : id === helga.decodeJid(helga.user.id) ?
            helga.user :
            (store.contacts[id] || {})
            return (withoutContact ? '' : v.name) || v.subject || v.verifiedName || PhoneNumber('+' + jid.replace('@s.whatsapp.net', '')).getNumber('international')
    }
    
    helga.sendContact = async (jid, kon, quoted = '', opts = {}) => {
	let list = []
	for (let i of kon) {
	    list.push({
	    	displayName: await helga.getName(i + '@s.whatsapp.net'),
	    	vcard: `BEGIN:VCARD\nVERSION:3.0\nN:${await helga.getName(i + '@s.whatsapp.net')}\nFN:${await helga.getName(i + '@s.whatsapp.net')}\nitem1.TEL;waid=${i}:${i}\nitem1.X-ABLabel:Ponsel\nitem2.EMAIL;type=INTERNET:helga995@gmail.com\nitem2.X-ABLabel:Email\nitem3.URL:https://saweria.co/helgaXd\nitem3.X-ABLabel:Instagram\nitem4.ADR:;;Indonesia;;;;\nitem4.X-ABLabel:Region\nEND:VCARD`
	    })
	}
	helga.sendMessage(jid, { contacts: { displayName: `${list.length} Kontak`, contacts: list }, ...opts }, { quoted })
    }
	
    helga.public = true

    helga.serializeM = (m) => smsg(helga, m, store)
    ...
    helga.ws.on('CB:call', async (json) => {
        const callerId = json.content[0].attrs['call-creator']
        if (json.content[0].tag == 'offer') {
        let pa7rick = await helga.sendContact(callerId, global.owner)
        helga.sendMessage(callerId, { text: `Sistem otomatis block!\nJangan menelpon bot!\nSilahkan Hubungi Owner Untuk Dibuka !`}, { quoted : pa7rick })
        helga.sendMessage(`${global.pemilik}`, {text: `Lapor Bot: Ada Yang Menelpon Bot`})
        await sleep(8000)
        await helga.updateBlockStatus(callerId, "block")
        }
        })
     ...
     * @returns
     */
    helga.send5ButImg = async (jid , text = '' , footer = '', img, but = [], options = {}) =>{
        let message = await prepareWAMessageMedia({ image: img }, { upload: helga.waUploadToServer })
        ...
        imageMessage: message.imageMessage,
            ...
            helga.relayMessage(jid, template.message, { messageId: template.key.id })
    }
     ...
     * @param {*} options 
     */
    helga.sendButtonText = (jid, buttons = [], text, footer, quoted = '', options = {}) => {
        let buttonMessage = {
            text,
            footer,
            buttons,
        ...
        helga.sendMessage(jid, buttonMessage, { quoted, ...options })
    }
	...
	helga.sendFile = async (jid, path, filename = '', caption = '', quoted, ptt = false, options = {}) => {
        let type = await helga.getFile(path, true)
        ...
        return await helga.sendMessage(jid, {
            ...options,
            caption,
            ptt,
            [mtype]: { url: pathFile },
    ...
    helga.downloadMediaMessage = async (message) => {
        let mime = (message.msg || message).mimetype || ''
        let messageType = message.mtype ? message.mtype.replace(/Message/gi, '') : mime.split('/')[0]
     ...
     * @returns 
     */
    helga.sendMedia = async (jid, path, fileName = '', caption = '', quoted = '', options = {}) => {
        let types = await helga.getFile(path, true)
       ...
       await helga.sendMessage(jid, { [type]: { url: pathFile }, caption, mimetype, fileName, ...options }, { quoted, ...options })
       }
    ...
    helga.cMod = (jid, copy, text = '', sender = helga.user.id, options = {}) => {
		...
		copy.key.fromMe = sender === helga.user.id

        return proto.WebMessageInfo.fromObject(copy)
    }
    
    helga.sendMediaAsSticker = async (jid, path, quoted, options = {}) => {
